...
class BatchRunner:
    def __init__(self, api_url: str = "http://localhost:8002", strategy_url: str = "http://localhost:8001"):
    ...
    async def run_batch(
        ...
        print(f"🚀 Starting batch run for {ticker} over {len(dates)} days ({dates[0]} to {dates[-1]})")
        ...
        for i, date in enumerate(dates):
            ...
            try:
                ...
                print(f"   ✅ Finished. Regime: {report.regime_detected or 'None'}. Trades: {report.total_trades}. PnL: {pnl_icon} ${report.total_pnl_dollars:.2f}")
    ...
    def print_batch_report(self, report: BatchReport):
        ...
        for day in report.daily_results:
            ...
            print(f"{day['date']:<12} | {str(day['regime'])[:10]:<10} | {day['trades']:<6} | {pnl_str:<10}")
