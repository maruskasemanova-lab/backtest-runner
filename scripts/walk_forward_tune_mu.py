...
async def run_backtest(
    ...
    try:
        ...
        if start_response.status_code != 200:
            ...
            return {"pnl_pct": 0, "trades": 0, "win_rate": 0, "sharpe": 0, "error": start_response.text[:200]}
        
        start_data = start_response.json()
        run_id = start_data.get("run_id", "")
        
        if not run_id:
            return {"pnl_pct": 0, "trades": 0, "win_rate": 0, "sharpe": 0, "error": "No run_id"}
        ...
        if play_response.status_code != 200:
            return {"pnl_pct": 0, "trades": 0, "win_rate": 0, "sharpe": 0, "error": f"Play failed: {play_response.text[:100]}"}
        ...
        return {"pnl_pct": 0, "trades": 0, "win_rate": 0, "sharpe": 0, "error": "Timeout"}
...
async def run_walk_forward_tuning():
    ...
    for i, r in enumerate(results[:10]):
        ...
        print(f"   Train: PnL={r['train']['pnl_pct']:.2f}%, trades={r['train']['trades']}, WR={r['train']['win_rate']:.1f}%")
        print(f"   Val:   PnL={r['val']['pnl_pct']:.2f}%, trades={r['val']['trades']}, WR={r['val']['win_rate']:.1f}%")
        print(f"   Strategies: {r['config']['enabled_strategies']}")
        print(f"   L2 Delta: {r['config']['l2_min_delta']}, Imbalance: {r['config']['l2_min_imbalance']}")
    ...
    if best_candidate:
        ...
        print(f"  Train: PnL={best_candidate['train']['pnl_pct']:.2f}%, trades={best_candidate['train']['trades']}")
        print(f"  Val:   PnL={best_candidate['val']['pnl_pct']:.2f}%, trades={best_candidate['val']['trades']}")
        print(f"  Test:  PnL={test_result['pnl_pct']:.2f}%, trades={test_result['trades']}")
