...
def analyze_profiles():
    ...
    valid_profiles = [p for p in profiles if p.get("metrics", {}).get("total_trades", 0) > 0]
    
    if valid_profiles:
        best_pnl = max(valid_profiles, key=lambda x: x.get("metrics", {}).get("total_pnl_pct", -999))
        print(f"\nBest PnL: {best_pnl.get('profile_id', 'N/A')}")
        print(f"   PnL: {best_pnl.get('metrics', {}).get('total_pnl_pct', 0):.2f}%")
        
        best_wr = max(valid_profiles, key=lambda x: x.get("metrics", {}).get("avg_win_rate_pct", 0))
        print(f"\nBest Win Rate: {best_wr.get('profile_id', 'N/A')}")
        print(f"   WR: {best_wr.get('metrics', {}).get('avg_win_rate_pct', 0):.1f}%")
        
        best_trades = max(valid_profiles, key=lambda x: x.get("metrics", {}).get("total_trades", 0))
    ...
    if valid_profiles:
        ...
        print(f"   Win Rate: {balanced.get('metrics', {}).get('avg_win_rate_pct', 0):.1f}%")
