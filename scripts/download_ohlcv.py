...
Downloads QQQ (cross-asset reference) for the evidence-based pipeline.
MU OHLCV already exists in ibkr-l2-script external data.
"""
import os
import sys
...
API_KEY = os.environ.get("DATABENTO_API_KEY", "db-V3DKB9puMkkJAEhN9UecTVHE8aqL8")
...
# Tickers and date ranges to download
DOWNLOADS = [
    ("QQQ", "2026-01-20", "2026-02-04"),
]


def download_range(client: db.Historical, ticker: str, start: str, end: str) -> bool:
    """Download OHLCV-1m for a ticker over a date range."""
    ...
    try:
        ...
        # Reset DatetimeIndex so ts_event becomes a column
        if hasattr(df.index, "name") and df.index.name:
            df = df.reset_index()
            # Rename ts_event to timestamp for compatibility with data_loader
...
def main():
    print("Downloading OHLCV-1m data from Databento")
