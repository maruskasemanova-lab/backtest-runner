...
- `session_summary.trades[]` preserves closed-trade diagnostics when present on strategy `position_closed`: `level_context`, `flow_snapshot`, `signal_metadata`, `break_even`, `entry_quality_diagnostics`, normalized `setup_type` for `level_fade`, and canonical audit/outcome fields (`size`, `position_notional_usd`, `gross_pnl_dollars`, `take_profit`, signal indexes, `trade_audit`).
- nested `trade_audit` verbosity is now config-driven from strategy `TradingConfig`: `trade_audit_level=off|core|full` and optional `trade_audit_fields[]` whitelist.
...
- session-only runtime metadata may be carried in the JSON body even when it is not part of `TradingConfig`; current supported payload is `heatmap_memory_catalog`.
...
- trade telemetry: `trade_audit_level` (`off|core|full`) and optional JSON-body `trade_audit_fields` whitelist for nested `trade_audit` projection
...
- structural-memory runtime payload: optional `heatmap_memory_catalog` (JSON body object only)
  - generated by runner from `daily_price_heatmap_levels`
  - keyed by `valid_for_day`
  - uses only rows with `source_as_of_date < valid_for_day` to avoid lookahead
  - each day payload carries D-1-or-earlier HVN/LVN zones (`zone_low|zone_high|zone_type|strength|age_days`)
  - strategy engine stores it outside canonical config, selects the matching day payload per session, and merges it into intraday levels as confluence/memory metadata rather than a standalone trade trigger
...
- Closed-position payloads include flow diagnostics (`flow_strategy`, `book_pressure_confirmed`, `book_pressure_avg`, `book_pressure_trend`, `signed_aggression`, `flow_snapshot`), preserved `level_context`/`signal_metadata`, `break_even`, `entry_quality_diagnostics` (first-bar stop-loss analysis tags + entry confluence/risk snapshot), explicit outcome fields (`size`, `position_notional_usd`, `gross_pnl_dollars`, `cost_usd`, `cost_pct`, `take_profit`, signal indexes), and canonical nested `trade_audit` (`outcome`, `setup`, `level`, `flow`, `risk_execution`).
- when `trade_audit_level=full`, nested `trade_audit.raw` also carries raw observability blocks (`signal_metadata`, `flow_snapshot`, `level_context`, `break_even`, `entry_quality_diagnostics`) so consumers can read one canonical audit tree instead of inferring from scattered top-level fields.
- Session summary now includes `entry_timing_diagnostics` and per-strategy `vwap_magnet_entry_timing_diagnostics` for fast-stop analysis, and `session_summary.trades[]` mirrors the same preserved close-trade diagnostics plus config-shaped `trade_audit` so runner-side summaries do not drop audit context.
