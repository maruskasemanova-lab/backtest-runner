...
class WalkForwardRunner:
    ...
    def __init__(self, config: WalkForwardConfig):
        ...
        self.tester = StrategyTester(
            api_url=config.api_url, strategy_api_url=config.strategy_api_url
        )
    ...
    def _build_start_overrides(self) -> Dict[str, Any]:
        ...
        mode = (
            str(self.config.strategy_selection_mode or "adaptive_top_n").strip().lower()
        )
    ...
    async def run_single_day(self, ticker: str, date: str) -> DailyResult:
        ...
        try:
            ...
            for trade in report.trades:
                flow_snapshot = (
                    trade.flow_snapshot if isinstance(trade.flow_snapshot, dict) else {}
                )
                signal_metadata = (
            ...
            if self.config.verbose:
                ...
                print(
                    f"   {icon} Regime: {result.regime or 'None':10} | "
    ...
    def _generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "config": {
                ...
                "strategy_selection_mode": self._build_start_overrides()[
                    "strategy_selection_mode"
                ],
                "max_active_strategies": self._build_start_overrides()[
            ...
            "summary": {
                ...
                "avg_trades_per_day": (
                    round(total_trades / len(self.results), 2) if self.results else 0
                ),
                "avg_pnl_per_day": (
                    round(total_pnl / len(self.results), 2) if self.results else 0
                ),
            },
    ...
    def _print_summary(self, report: Dict[str, Any]):
        ...
        for ticker, data in report["by_ticker"].items():
            ...
            print(
                f"  {ticker:6} | Trades: {data['trades']:3} | PnL: {icon} ${data['pnl']:+.2f}"
            )
        ...
        for regime, data in report["by_regime"].items():
            icon = "🟢" if data["pnl"] >= 0 else "🔴"
            print(
                f"  {regime:10} | Days: {data['days']:3} | Trades: {data['trades']:3} | PnL: {icon} ${data['pnl']:+.2f}"
            )
...
async def main():
    ...
    parser.add_argument(
        "--start-date", "-s", default="2026-01-20", help="Start date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--end-date", "-e", default="2026-01-28", help="End date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--output", "-o", default="walk_forward_results", help="Output directory"
    )
    parser.add_argument(
        "--api-url", default="http://localhost:8002", help="Backtest runner API URL"
    )
    parser.add_argument(
