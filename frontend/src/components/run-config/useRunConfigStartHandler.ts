    ...
    async (e: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const payload: Record<string, any> = {
          ...
          tcbbo_gate_enabled: config.tcbbo_gate_enabled !== false,
          tcbbo_min_net_premium: Math.max(0, Number(config.tcbbo_min_net_premium ?? 0)),
          tcbbo_sweep_boost: Math.max(0, Number(config.tcbbo_sweep_boost ?? 5)),
          tcbbo_lookback_bars: Math.max(1, Math.trunc(Number(config.tcbbo_lookback_bars ?? 5))),
