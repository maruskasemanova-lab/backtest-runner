...
function DataManager({ downloadProgress }) {
    ...
    // Summary stats
    const stats = useMemo(() => {
        const tickers = new Set(catalogWithFormats.map((e) => e.ticker));
        const totalSize = catalogWithFormats.reduce((sum, e) => sum + (e.size_bytes || 0), 0);
        const totalRows = catalogWithFormats.reduce((sum, e) => sum + (e.row_count || 0), 0);
        return {
                    ...
                    <div className="card-body dm-table-container">
                        {!loading && catalogWithFormats.length === 0 ? (
                            <div className="dm-empty">
                                ...
                                <div>No data found. Click &quot;Scan&quot; to detect existing files or download new data.</div>
                            </div>
