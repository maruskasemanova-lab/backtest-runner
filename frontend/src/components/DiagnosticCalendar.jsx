...
const hasAdaptiveProfileSources = (dayResult) => {
  ...
  if (Array.isArray(dayResult?.runs)) {
    const hasRunMatch = dayResult.runs.some((run) => {
      ...
      const runProfile = String(run?.adaptive_profile_id || "").trim();
...
function DiagnosticCalendar() {
  ...
  useEffect(() => {
    ...
    const fetchReport = async () => {
      ...
      try {
        ...
        if (queryAdaptiveProfileId) params.set("adaptive_profile_id", queryAdaptiveProfileId);
  ...
  const selectedProfileList = selectedResult ? formatAdaptiveProfileList(selectedResult) : null;
  ...
  const adaptiveProfileOptions = useMemo(() => {
    const raw = Array.isArray(report?.filter_options?.adaptive_profiles)
  ...
  const reportPath = useMemo(() => {
    ...
    if (queryAdaptiveProfileId) segments.push(`adaptive=${queryAdaptiveProfileId}`);
          ...
          <form className="diagnostic-toolbar" onSubmit={handleSubmit}>
            ...
            <label>
              ...
              <select value={tradeViewMode} onChange={(e) => setTradeViewMode(e.target.value)}>
                ...
                <option value="adaptive">Adaptive Trades</option>
              </select>
            </label>
                  ...
                  <>
                    ...
                    {selectedProfileList ? (
                      <div className="diagnostic-row">
                        ...
                        <strong>{selectedProfileList}</strong>
                      </div>
                    ) : null}
                        ...
                        {selectedRuns.map((run, index) => (
                          ...
                          >
                            ...
                            <div className="diagnostic-trade-content">
                              ...
                              {String(run?.adaptive_profile_id || run?.adaptive_profile_name || "").trim() ? (
                                <div className="diagnostic-row">
                                  <span>Adaptive Profile</span>
                                  <strong>
                                    {[
                                      String(run?.adaptive_profile_id || "").trim(),
                                      String(run?.adaptive_profile_name || "").trim(),
                                    ].filter(Boolean).join(" | ")}
                                  </strong>
                                </div>
                              ) : null}
