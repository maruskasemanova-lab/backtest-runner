...
def analyze_intraday_trends(data_path: str = None):
    """
    Analyze intraday price patterns to identify short-term trends.
    
    Uses 1-minute OHLCV data to detect:
    - Morning momentum (9:30-10:30)
    - Midday consolidation
    - Afternoon breakout/breakdown
    """
    
    # For demo, analyze based on trade results
    results_dir = Path("aos_walk_forward_results")
    ...
    for date, day_df in df.groupby('date'):
        ...
        same_side_pnl = sum(t['pnl_dollars'] for t in subsequent_same_side) if subsequent_same_side else 0
        opposite_pnl = sum(t['pnl_dollars'] for t in subsequent_opposite) if subsequent_opposite else 0
        ...
        daily_patterns.append({
            ...
            'trend': 'BULLISH' if longs_pnl > shorts_pnl else 'BEARISH' if shorts_pnl > longs_pnl else 'NEUTRAL'
        })
    ...
    for _, row in patterns_df.iterrows():
        trend_icon = "📈" if row['trend'] == 'BULLISH' else "📉" if row['trend'] == 'BEARISH' else "➡️"
        pnl_icon = "🟢" if row['total_pnl'] >= 0 else "🔴"
        print(f"  {row['date']} | {trend_icon} {row['trend']:8} | "
    ...
    print(f"    Win Rate: {len(longs[longs['pnl_dollars'] > 0]) / len(longs) * 100:.1f}%" if len(longs) > 0 else "0%")
    
    print(f"\n  SHORT trades:")
    print(f"    Count: {len(shorts)}")
    print(f"    Total PnL: ${shorts['pnl_dollars'].sum():+.2f}")
    print(f"    Win Rate: {len(shorts[shorts['pnl_dollars'] > 0]) / len(shorts) * 100:.1f}%" if len(shorts) > 0 else "0%")
    ...
    if len(first_losers) > 0:
        if first_losers['same_side_pnl'].mean() < 0 and first_losers['opposite_pnl'].mean() > 0:
