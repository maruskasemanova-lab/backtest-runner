...
class PatternLibraryManager:
        ...
        for path in self.library_dir.glob(pattern):
            try:
                ...
                libraries.append({
                    ...
                    "total_patterns": len(library.cluster_patterns) + len(library.sequential_patterns),
    ...
    def create_library(
        ...
        best_pattern = max(all_patterns, key=lambda p: p.win_rate * p.support) if all_patterns else None
    ...
    def merge_libraries(
        ...
        Args:
            ...
            ticker: Ticker for merged library
            version: Version for merged library
            
        Returns:
            Merged PatternLibrary
        """
        all_cluster_patterns = []
        all_sequential_patterns = []
        total_snapshots = 0
        config = {}
        
        seen_cluster_ids = set()
        seen_sequential_ids = set()
        
        for library in libraries:
            # Add cluster patterns (deduplicate by ID)
            for pattern in library.cluster_patterns:
                if pattern.pattern_id not in seen_cluster_ids:
                    all_cluster_patterns.append(pattern)
                    seen_cluster_ids.add(pattern.pattern_id)
            
            # Add sequential patterns (deduplicate by ID)
            for pattern in library.sequential_patterns:
                if pattern.pattern_id not in seen_sequential_ids:
                    all_sequential_patterns.append(pattern)
                    seen_sequential_ids.add(pattern.pattern_id)
            
            total_snapshots += library.total_snapshots_analyzed
            config.update(library.discovery_config)
        
        return PatternLibrary(
            ticker=ticker.upper(),
            version=version,
            cluster_patterns=all_cluster_patterns,
    ...
    def delete_library(
        ...
        try:
            path.unlink()
            logger.info(f"Deleted pattern library {path}")
            return True
        except Exception as e:
            logger.error(f"Failed to delete library {path}: {e}")
            return False
...
def get_pattern_library_path(
    ticker: str,
    version: str = "v1",
    library_dir: Path = DEFAULT_LIBRARY_DIR,
) -> Path:
    """
    Get the standard path for a pattern library file.
    ...
    """
    return Path(library_dir) / f"{ticker.upper()}_patterns_{version}.json"
