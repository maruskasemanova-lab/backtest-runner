...
class PatternInput(BaseModel):
    ...
    l2_flow_score_z: float = Field(default=0.0, description="Composite flow score z-score")
    
    # === Regime Context ===
    regime: str = Field(default="MIXED", description="Market regime: TRENDING|CHOPPY|MIXED")
    trend_efficiency: float = Field(default=0.5, ge=0.0, le=1.0, description="Trend efficiency 0-1")
    volatility_pct: float = Field(default=0.5, ge=0.0, le=1.0, description="Volatility percentile rank")
    
    # === Time Context ===
    hour_of_day: int = Field(default=10, ge=0, le=23, description="Hour of day (market hours)")
    minute_of_hour: int = Field(default=0, ge=0, le=59, description="Minute of hour")
    day_of_week: int = Field(default=1, ge=0, le=6, description="Day of week (0=Monday)")
    
    # === Price Action Context ===
    bar_body_ratio: float = Field(default=0.5, ge=0.0, le=1.0, description="|close-open| / (high-low)")
    close_location: float = Field(default=0.5, ge=0.0, le=1.0, description="(close-low) / (high-low)")
    ...
    def to_state_label(self) -> str:
        ...
        momentum_state = "up" if self.momentum_z > 0.5 else "down" if self.momentum_z < -0.5 else "neutral"
        delta_state = "spike_up" if self.l2_delta_z > 1.5 else "spike_down" if self.l2_delta_z < -1.5 else "normal"
        vol_state = "high" if self.volume_z > 1.0 else "low" if self.volume_z < -1.0 else "normal"
...
class MatchConfig(BaseModel):
    ...
    max_matches: int = Field(default=3, ge=1, le=10, description="Maximum matches to return")
    direction_filter: str = Field(default="any", description="Filter by direction: any|bullish_only|bearish_only")
    min_win_rate: float = Field(default=0.50, ge=0.0, le=1.0, description="Minimum historical win rate")
    min_support: int = Field(default=5, ge=1, description="Minimum pattern support count")
...
class PatternMatch(BaseModel):
    ...
    # === Match Quality ===
    similarity_score: float = Field(ge=0.0, le=1.0, description="How well current state matches pattern")
    distance: float = Field(ge=0.0, description="Euclidean distance to pattern centroid")
    
    # === Historical Performance ===
    historical_win_rate: float = Field(ge=0.0, le=1.0, description="Win rate when pattern occurred")
    ...
    confidence_interval: Tuple[float, float] = Field(default=(0.0, 1.0), description="95% CI for win rate")
    
    # === Direction Signal ===
    direction: Direction = Field(description="Trading direction: bullish|bearish|neutral")
    ...
    recommended_stop_atr: float = Field(default=1.5, ge=0.1, description="ATR multiplier for stop")
    recommended_target_rr: float = Field(default=2.0, ge=0.5, description="Risk-reward ratio for target")
    
    # === Evidence for Decision Engine ===
    evidence_strength: float = Field(ge=0.0, le=100.0, description="Strength 0-100 for EvidenceSource")
    evidence_reasoning: str = Field(default="", description="Human-readable explanation")
...
class PatternOutcome(BaseModel):
    ...
    bars_held: int = Field(default=0, ge=0, description="Number of bars position was held")
    max_favorable_excursion: float = Field(default=0.0, description="Maximum favorable excursion %")
    max_adverse_excursion: float = Field(default=0.0, description="Maximum adverse excursion %")
...
class ClusterPattern(BaseModel):
    ...
    feature_names: List[str] = Field(default_factory=list, description="Feature names for centroid")
    ...
    long_ratio: float = Field(default=0.5, ge=0.0, le=1.0, description="Ratio of long signals")
    
    # === Feature Importance ===
    feature_importance: Dict[str, float] = Field(default_factory=dict, description="Feature importance scores")
    
    # === Recommended Parameters ===
    recommended_stop_atr: float = Field(default=1.5, description="Recommended ATR stop multiplier")
    recommended_target_rr: float = Field(default=2.0, description="Recommended risk-reward ratio")
...
class PatternLibrary(BaseModel):
    ...
    # === Metadata ===
    best_pattern_id: str = Field(default="", description="ID of best performing pattern")
    coverage_ratio: float = Field(default=0.0, ge=0.0, le=1.0, description="Ratio of bars with pattern matches")
    
    def get_pattern_by_id(self, pattern_id: str) -> Optional[ClusterPattern | SequentialPattern]:
        """Get pattern by ID."""
        for p in self.cluster_patterns:
...
class DiscoveryConfig(BaseModel):
    ...
    clustering_k_range: Tuple[int, int] = Field(default=(50, 200), description="Range of k values to try")
    clustering_method: str = Field(default="kmeans", description="Clustering method: kmeans|gmm")
    
    # === Sequential Mining ===
    sequential_enabled: bool = Field(default=True)
    n_gram_range: Tuple[int, int] = Field(default=(2, 5), description="N-gram length range")
    
    # === Filtering ===
    min_support: int = Field(default=10, ge=1, description="Minimum pattern support")
    min_win_rate: float = Field(default=0.55, ge=0.0, le=1.0, description="Minimum win rate")
    min_sharpe: float = Field(default=1.0, description="Minimum Sharpe ratio")
    
    # === Feature Extraction ===
    lookback_bars: int = Field(default=20, ge=1, description="Lookback bars for context")
...
class EvidenceRequest(BaseModel):
    """Request for pattern evidence."""
    ticker: str
    feature_vector_json: str
    recent_states_json: str = Field(default="[]")
