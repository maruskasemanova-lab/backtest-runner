...
class SequentialPatternMiner:
    ...
    def mine(
        ...
        for n in range(n_min, n_max + 1):
            ngram_outcomes: Dict[Tuple[str, ...], List[PatternOutcome]] = defaultdict(list)
        ...
        unique_patterns.sort(
            key=lambda p: p.win_rate * math.log(p.support + 1),
            reverse=True
        )
...
def match_sequence(
    ...
    """
    Check if recent states match a sequential pattern.
    
    Args:
        pattern: SequentialPattern to match
        recent_states: List of recent state labels
        
    Returns:
        True if the pattern matches
    """
    return pattern.matches(recent_states)
...
def compute_sequence_similarity(
    ...
    """
    Compute similarity between recent states and pattern sequence.
    
    Uses partial matching for incomplete sequences.
    """
    if len(recent_states) < len(pattern.sequence):
        # Partial match - check if recent states match the beginning
        match_length = len(recent_states)
        pattern_prefix = pattern.sequence[:match_length]
        
        if recent_states == pattern_prefix:
            return match_length / len(pattern.sequence)
        return 0.0
    
    # Full match check
    if pattern.matches(recent_states):
        return 1.0
    
    # Check for partial suffix match
    for i in range(1, len(pattern.sequence)):
        if recent_states[-(len(pattern.sequence) - i):] == pattern.sequence[i:]:
            return (len(pattern.sequence) - i) / len(pattern.sequence)
