...
def load_trades_from_csv(csv_path: str) -> pd.DataFrame:
    ...
    df['time_period'] = df.apply(lambda row: get_time_period(row['entry_hour'], row['entry_minute']), axis=1)
...
def find_optimal_time_windows(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {
        ...
        'recommended_session': 'MORNING' if morning_pnl > afternoon_pnl else 'AFTERNOON',
        'hourly_breakdown': {
...
def generate_time_filter_recommendations(analysis: Dict[str, Any]) -> List[str]:
    ...
    if optimal['morning_pnl'] > 0 and optimal['afternoon_pnl'] <= 0:
        recommendations.append(f"Trade MORNING session only (9:30-11:30): +${optimal['morning_pnl']}")
        ...
        recommendations.append(f"Trade AFTERNOON session only (14:00-16:00): +${optimal['afternoon_pnl']}")
        recommendations.append(f"SKIP morning session: ${optimal['morning_pnl']}")
    else:
        recommendations.append(f"Morning: ${optimal['morning_pnl']}, Afternoon: ${optimal['afternoon_pnl']}")
...
def run_time_analysis(csv_path: str = None):
    ...
    for period, data in analysis['by_time_period'].items():
        ...
        print(f"\n{period}:")
        print(f"  {icon} PnL: ${data['total_pnl']:+.2f} | "
    ...
    if first_trade:
        ...
        for key, data in first_trade.items():
            print(f"    {key}: subsequent avg = ${data['subsequent_avg']:+.2f} ({data['count']} days)")
    ...
    # Save analysis
    output_path = Path("aos_walk_forward_results") / "time_analysis.json"
